...
const Game = {
  ...
  checkAutoReset() {
    setInterval(async () => {
      try {
        const gameData = (await db.ref('game').get()).val() || {};
...
db.ref('game/winner').on('value', async snap => {
  ...
  if (win && !win.processed) {
    ...
    // ✅ FIXED JACKPOT CALCULATION (NO CODE REMOVED)
    const boardsSnap = await db.ref('reserved_boards').get();
    let pool = 0;

    if (boardsSnap.exists()) {
      boardsSnap.forEach(child => {
        pool += child.val().betAmount || 0;
      });
    }
    ...
    if (pool > 0) {
      ...
      await db.ref(`users/${win.id}/bal`)
        .transaction(c => (c || 0) + winnerPrize);

      await db.ref(`users/${ADMIN_ID}/bal`)
        .transaction(c => (c || 0) + adminShare);
      ...
      // Jackpot snapshot (read-only for clients)
      await db.ref('game/jackpot').set(winnerPrize);
    }
  ...
  if (txMatch && amtMatch && phMatch) {
    ...
    try {
      ...
      if (userSnap.exists()) {
        ...
        await db.ref(`used_transactions/${txId}`)
          .set({ uid, amount, date: new Date().toISOString() });
      }
    } catch (e) {

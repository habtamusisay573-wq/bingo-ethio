    ...
    if (win && !win.processed) {
        ...
        if (boardsSnap.exists()) {
            boardsSnap.forEach(child => {
                pool += (child.val().betAmount || 0); // የእያንዳንዱን ካርቴላ ትክክለኛ ዋጋ መደመር
            });
        }
...
await db.ref(history/${win.id}).push({
            ...
            // --- የዳኛ ስጦታ (Gift Box) Logic ---
            boardsSnap.forEach(async (child) => {
                const player = child.val();
                // 15% እድል ለተሸነፉ ተጫዋቾች
                if (player.userId !== win.id && Math.random() < 0.15) {
                    const giftAmount = Math.floor(Math.random() * 10) + 5; 
                    await db.ref(users/${player.userId}/bal).transaction(c => (c || 0) + giftAmount);
                    await db.ref(game/judge_gift).set({
                        uid: player.userId,
                        amt: giftAmount,
                        ts: Date.now()
                    });
        ...
        setTimeout(async () => {
            await db.ref('game/judge_gift').remove();
            Game.forceReset();
        }, 5000);
    }
});
